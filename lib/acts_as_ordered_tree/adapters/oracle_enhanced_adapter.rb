    ...
    module OracleEnhancedAdapter
      ...
      def self_and_ancestors
        if persisted? && !send("#{parent_column}_changed?")
          query = <<-QUERY
            ...
            UNION ALL
            SELECT alias1.id, alias1.#{parent_column}, self_and_ancestors.position + 1
            FROM #{self.class.quoted_table_name} alias1
              INNER JOIN self_and_ancestors ON alias1.id = self_and_ancestors.parent_id
          QUERY

          recursive_scope.with_recursive("self_and_ancestors", query).
                          order("self_and_ancestors.position DESC")
        else
          ancestors + [self]
        end
      end
      ...
      def ancestors
        query = <<-QUERY
          ...
          FROM #{self.class.quoted_table_name} alias1
            INNER JOIN ancestors ON alias1.id = ancestors.parent_id
        QUERY
      ...
      def self_and_descendants
        ...
        relation = recursive_scope.with_recursive("descendants", query).order("p_order")
